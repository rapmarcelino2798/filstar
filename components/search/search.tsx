...
export default function SearchBar() {
  ...
  return (
    <div className="w-full max-w-4xl p-3 md:p-2.5 bg-[#171a22]/90 backdrop-blur-xl border border-white/10 rounded-3xl md:rounded-full shadow-2xl flex flex-col md:flex-row items-center gap-4 md:gap-0 justify-between text-white">
      
      {/* Left: Buy / Rent Switcher */}
      <div className="flex items-center justify-center w-full md:w-auto bg-[#101217] p-1 rounded-full border border-white/5">
        <button
          onClick={() => setActiveTab("buy")}
          className={`flex-1 md:flex-none px-6 py-2 rounded-full text-sm font-medium transition-all duration-200 ${
            activeTab === "buy"
              ? "bg-[#24211b] text-[#c9a86a] border border-[#c9a86a]/30 shadow-md"
      ...
      <div className="flex items-center justify-between md:justify-start w-full md:w-auto gap-3 px-4 py-2 md:py-0 bg-white/5 md:bg-transparent rounded-2xl md:rounded-none cursor-pointer text-gray-300 hover:text-white transition-colors">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium whitespace-nowrap">Min-Max Price</span>
        </div>
